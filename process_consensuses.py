...
def process_consensuses(in_dirs, fat, initial_descriptor_dir):
    """For every input consensus, finds the descriptors published most recently before the descriptor times listed for
    the relays in that consensus, records state changes indicated by descriptors published during the consensus fresh
    period, and writes out pickled consensus and descriptor objects with the relevant information.
        Inputs:
...
def process_one_consensus(desc_out_dir, descriptors, fat, pathname):
    ...
    i = 0
    for document in stem.descriptor.parse_file(cons_f, validate=True, document_handler='DOCUMENT'):
        if i > 0:
            raise ValueError('Unexpectedly found more than one consensus in file: {}'.format(pathname))
        if cons_valid_after is None:
        ...
        if cons_fresh_until is None:
            cons_fresh_until = document.fresh_until
            # compute timestamp version once here
            fresh_until_ts = pathsim.timestamp(cons_fresh_until)
        if not fat:
            if cons_bw_weights is None:
                cons_bw_weights = document.bandwidth_weights
            if (cons_bwweightscale is None) and ('bwweightscale' in document.params):
                cons_bwweightscale = document.params['bwweightscale']
            for fprint, r_stat in document.routers.iteritems():
                relays[fprint] = pathsim.RouterStatusEntry(fprint, r_stat.nickname, r_stat.flags, r_stat.bandwidth)
        consensus = document
        i += 1
    ...
    for fprint, r_stat in consensus.routers.iteritems():
        ...
        if r_stat.fingerprint in descriptors:
            for t, d in descriptors[r_stat.fingerprint].items():
                ...
                if (valid_after_ts - t < pathsim.TorOptions.router_max_age) and \
                        (t <= pub_time) and (t > desc_time) and (t <= fresh_until_ts):
                    desc_time = t
                ...
                if desc_time_fresh is None:
                    ...
                    if (t > desc_time_fresh) and (t <= valid_after_ts):
                        desc_time_fresh = t
                elif t < desc_time_fresh:
                    desc_time_fresh = t
    ...
    if (cons_valid_after is not None) and (cons_fresh_until is not None):
        if not fat:
            consensus_out = pathsim.NetworkStatusDocument(cons_valid_after, cons_fresh_until,
                                                          cons_bw_weights, cons_bwweightscale, relays)
        else:
            if type_annotation is not None:
                consensus_out = type_annotation + str(consensus)
            else:
                consensus_out = str(consensus)
        hibernating_statuses.sort(key=lambda x: x[0], reverse=True)
        outpath = os.path.join(desc_out_dir, cons_valid_after.strftime('%Y-%m-%d-%H-%M-%S-network_state'))
        f = open(outpath, 'wb')
